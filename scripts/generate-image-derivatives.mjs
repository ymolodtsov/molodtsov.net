...
function convert(source, output, width, quality) {
...
if (!fs.existsSync(cwebp)) {
  throw new Error(`cwebp not found at ${cwebp}`);
}
...
for (const source of walk(uploadsRoot)) {
  for (const derivative of [
    ...
    const result = convert(source, outputPath(source, derivative.suffix), derivative.width, derivative.quality);
    if (result.skipped) {
      skipped += 1;
    } else {
      created += 1;
